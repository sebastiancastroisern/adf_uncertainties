...
def npy_files_builder(file_path: str, data_filepath: str) -> None:
    ...
    assert np.array_equal(
        np.unique(event_idx_coord), np.unique(event_idx_coinc)
    ...
    np.save(data_filepath + "/an_nants.npy", len(event_idx_coord))
...
def main():
    ...
    nants, antenna_coords_array, peak_time_array_m, peak_time_array_s, peak_amp_array, ncoincs, events_ids = lo.load_data_files_np(file_path)
    _, idx = np.unique(events_ids, return_index=True)
    events_ids_unique = events_ids[np.sort(idx)] # Unique event ids in the same order as the first occurrence in events_ids
